...
__all__ = ["DCAE", "dc_ae_f16", "dc_ae_f16_res", "dc_ae_f32", "dc_ae_f64c128", "dc_ae_f128c512"]
...
def dc_ae_f16(name: str, channels, pretrained_path: str) -> DCAEConfig:
    if name in ['dc-ae-f16-in-1.0']:
        cfg_str = (
            ...
            "decoder.norm=[bn2d,bn2d,bn2d,ln2d,ln2d] decoder.act=[relu,relu,relu,silu,silu]"
        )
    else:
        raise NotImplementedError
...
def dc_ae_f16_res(name: str, channels, pretrained_path: str) -> DCAEConfig:
    if name in ['dc-ae-f16-res-in-1.0']:
        cfg_str = (
            f"latent_channels={channels} "
            "encoder.block_type=[ResBlock,ResBlock,ResBlock,ResBlock,ResBlock] "
            "encoder.width_list=[128,256,512,512,1024] encoder.depth_list=[0,4,8,4,4] "
            "decoder.block_type=[ResBlock,ResBlock,ResBlock,ResBlock,ResBlock] "
            "decoder.width_list=[128,256,512,512,1024] decoder.depth_list=[0,5,10,4,4] "
            "decoder.norm=[bn2d,bn2d,bn2d,ln2d,ln2d] decoder.act=[relu,relu,relu,silu,silu]"
        )
    else:
        raise NotImplementedError
...
def dc_ae_f32(name: str, channels, pretrained_path: str) -> DCAEConfig:
    if name in ["dc-ae-f32-in-1.0", "dc-ae-f32-mix-1.0"]:
        cfg_str = (
            f"latent_channels={channels} "
            ...
            "decoder.norm=[bn2d,bn2d,bn2d,ln2d,ln2d,ln2d] decoder.act=[relu,relu,relu,silu,silu,silu]"
        )
    else:
        raise NotImplementedError
    cfg = OmegaConf.from_dotlist(cfg_str.split(" "))
...
def dc_ae_f64c128(name: str, pretrained_path: Optional[str] = None) -> DCAEConfig:
    if name in ["dc-ae-f64c128-in-1.0", "dc-ae-f64c128-mix-1.0"]:
        cfg_str = (
            ...
            "decoder.norm=[bn2d,bn2d,bn2d,ln2d,ln2d,ln2d,ln2d] decoder.act=[relu,relu,relu,silu,silu,silu,silu]"
        )
    else:
        raise NotImplementedError
    cfg = OmegaConf.from_dotlist(cfg_str.split(" "))
...
def dc_ae_f128c512(name: str, pretrained_path: Optional[str] = None) -> DCAEConfig:
    if name in ["dc-ae-f128c512-in-1.0", "dc-ae-f128c512-mix-1.0"]:
        cfg_str = (
            ...
            "decoder.norm=[bn2d,bn2d,bn2d,ln2d,ln2d,ln2d,ln2d,ln2d] decoder.act=[relu,relu,relu,silu,silu,silu,silu,silu]"
        )
    else:
        raise NotImplementedError
    cfg = OmegaConf.from_dotlist(cfg_str.split(" "))
