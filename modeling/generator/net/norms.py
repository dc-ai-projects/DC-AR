...
def build_norm(name="bn2d", num_features=None, affine=True, **kwargs) -> nn.Module:
    if name in ["ln", "ln2d"]:
...
def get_norm_name(norm: nn.Module) -> str:
    if norm is None:
        return None
    module2name = {}
...
def reset_bn(
    ...
    from apps.utils import AverageMeter, is_master, sync_tensor, get_device, list_join
...
def set_norm_eps(model: nn.Module, eps: float = None, momentum: float = None) -> None:
    for m in model.modules():
