...
from .import_utils import is_xformers_available
...
class FlashAttention(Attention_):
    ...
    def forward(self, x, mask=None, HW=None, block_id=None, attn_bias=None):
        B, N, C = x.shape
        ...
        if _xformers_available:
            if attn_bias is not None and mask is None:
                attn_bias = attn_bias.expand(B, self.num_heads, -1, -1).to(dtype=q.dtype)
            x = xformers.ops.memory_efficient_attention(q, k, v, p=self.attn_drop.p, attn_bias=attn_bias)
        else:
            q, k, v = q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2)
            if mask is not None and mask.ndim == 2:
            ...
            if attn_bias is not None:
                x = F.scaled_dot_product_attention(q, k, v, attn_mask=attn_bias, dropout_p=0.0, is_causal=False)
            else:
                x = F.scaled_dot_product_attention(q, k, v, attn_mask=mask, dropout_p=0.0, is_causal=False)
            x = x.transpose(1, 2)
...
class CaptionEmbedder(nn.Module):
    ...
    def forward(self, caption, train, uncond_prob):
        ...
        if isinstance(uncond_prob, torch.Tensor):
            use_dropout = True
        else:
